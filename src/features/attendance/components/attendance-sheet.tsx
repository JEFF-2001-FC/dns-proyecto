...
import { ATTENDANCE_OPTIONS, type AttendanceStatus, type ClanOption, type RosterItem } from "../types";
...
export function AttendanceSheet({ agapeId, meetingTypeId, date, initialTopic, roster, clans, editable, readOnlyReason }: Props) {
  ...
  const visible = useMemo(() => {
    ...
    return people.filter(
      (p) => (clanFilter === "all" || p.clanId === clanFilter) && (!q || p.fullName.toLowerCase().includes(q)),
    );
  }, [people, clanFilter, query]);

  const totals = useMemo(() => {
    const t = { present: 0, late: 0, absent: 0, justified: 0 } as Record<AttendanceStatus, number>;
  ...
  const markVisible = (status: AttendanceStatus) => {
    setMarks((m) => ({ ...m, ...Object.fromEntries(visible.map((p) => [p.id, status])) }));
    setDirty(true);
  };
  ...
  const save = () =>
    startSaving(async () => {
      const res = await saveAttendance({
        ...
        date,
        topic,
        records: people.map((p) => ({ adolescentId: p.id, status: marks[p.id] ?? "present" })),
      });
  ...
  const onVisitorAdded = (item: RosterItem) => {
    setPeople((list) => [...list, item].sort((a, b) => a.lastName.localeCompare(b.lastName, "es")));
    setMarks((m) => ({ ...m, [item.id]: "present" }));
    setFeedback({ ok: true, text: `${item.fullName} agregado y marcado presente.` });
  };

  return (
    <div className="pb-28">
      <label className="block text-sm">
        <span className="mb-1 block font-medium text-zinc-700">Tema de la reunión</span>
        <input
          value={topic}
          onChange={(e) => {
      ...
      {!editable && readOnlyReason && (
        <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{readOnlyReason}</p>
      )}

      <div className="mt-5 flex flex-wrap items-center gap-2">
        <div className="relative min-w-0 flex-1">
          <Search aria-hidden className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400" />
          <input
            type="search"
            value={query}
      ...
      <div className="-mx-1 mt-3 flex gap-2 overflow-x-auto px-1 pb-1" role="group" aria-label="Filtrar por clan">
        {[{ id: "all", name: "Todos", color: "#18181b" }, ...clans].map((c) => {
          ...
          return (
            ...
            >
              {c.id !== "all" && <span aria-hidden className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: c.color }} />}
              {c.name}
            </button>
          );
        })}
      ...
      {editable && visible.length > 1 && (
        <div className="mt-3 flex items-center gap-2 text-sm text-zinc-600">
          ...
          <button type="button" onClick={() => markVisible("present")} className="font-medium text-emerald-700 underline-offset-2 hover:underline">
            presentes
          </button>
          <button type="button" onClick={() => markVisible("absent")} className="font-medium text-red-700 underline-offset-2 hover:underline">
            faltas
          </button>
        </div>
      )}
      ...
      {people.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-center">
          ...
          <p className="mt-1 text-sm text-zinc-500">Usa “Nuevo” para registrar a los asistentes de hoy.</p>
        </div>
      ) : (
        <ul className="mt-4 divide-y divide-zinc-100 overflow-hidden rounded-2xl border border-zinc-200 bg-white">
          {visible.map((p) => (
            <li key={p.id} className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-center gap-3">
                <span aria-hidden className="h-8 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: p.clanColor ?? "#d4d4d8" }} />
                <div className="min-w-0">
                  <p className="truncate font-medium text-zinc-900">{p.fullName}</p>
                  <p className="text-xs text-zinc-500">
                    {p.clanName ?? "Sin clan"}
                    {p.status === "pending" && <span className="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-amber-800">Provisional</span>}
                  </p>
                </div>
              </div>

              <div role="radiogroup" aria-label={`Asistencia de ${p.fullName}`} className="grid grid-cols-4 gap-1.5 sm:w-80">
                  ...
                  return (
                    <button
                      ...
                      className={`h-11 rounded-xl border text-xs font-semibold transition-colors disabled:opacity-60 ${
                        selected ? TONE[o.value] : "border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50"
                      }`}
                    >
          ...
          {visible.length === 0 && <li className="p-6 text-center text-sm text-zinc-500">Nadie coincide con el filtro.</li>}
        </ul>
      )}

      {/* Barra fija inferior: resumen + guardar */}
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-200 bg-white/95 px-4 py-3 backdrop-blur lg:left-64">
        <div className="mx-auto flex max-w-5xl items-center gap-3">
          <div className="min-w-0 flex-1 text-xs text-zinc-600">
            ...
            <p className="truncate" role="status">
              {feedback ? (
                <span className={feedback.ok ? "text-emerald-700" : "text-red-700"}>{feedback.text}</span>
              ) : (
          ...
          <button
            type="button"
            onClick={save}
            disabled={!editable || saving || people.length === 0 || topic.trim().length < 2}
      ...
      {editable && (
        <ExpressRegisterDialog
          ...
          clans={clans}
          meeting={{ agapeId, meetingTypeId, date, topic: topic.trim() || initialTopic }}
