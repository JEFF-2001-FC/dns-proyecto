...
export default async function ReportesPage({ searchParams }: { searchParams: Promise<{ agape?: string; from?: string; to?: string }> }) {
  await requireRole("admin");
  const supabase = await createClient();
  const filters = await searchParams;
  const { data: agapes } = await supabase.from("agapes").select("id, name").eq("active", true).order("name");
  const query = new URLSearchParams(); if (filters.agape) query.set("agape", filters.agape); if (filters.from) query.set("from", filters.from); if (filters.to) query.set("to", filters.to);
  const masterHref = `/api/reportes/maestro.xlsx${query.size ? `?${query}` : ""}`;
