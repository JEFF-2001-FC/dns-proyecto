...
export default async function MaterialesPage() {
  ...
  const fileItems = (data ?? []).filter((item) => item.storage_path);
  const admin = fileItems.length ? createAdminClient() : null;
  const items = await Promise.all((data ?? []).map(async (item) => {
    const signed = item.storage_path && admin ? await admin.storage.from("dns-materials").createSignedUrl(item.storage_path, 900) : null;
