...
export default async function EventosPage() {
  ...
  const [eventsResult, agapesResult, clansResult] = await Promise.all([
    supabase.from("events").select("id, title, description, starts_at, ends_at, location, scope, approval_status, agape:agapes(name), clan:clans(name)").order("starts_at", { ascending: true }).limit(100),
